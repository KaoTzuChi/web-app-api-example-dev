...
class db_util:
    ...
    def read_document_byfield(self, fieldname, fieldvalue):
        try:
            _db = self.get_database()
            _colname = self.get_collection_name()
            _result = None
            if _db:
                if _colname:
                    _collection = _db[_colname]
                    if _collection:
                        _cursor = _collection.find({fieldname: fieldvalue})                   
            return _cursor     
        except Exception as e:
            print('====== %s.%s: %s' % ('db_util','read_document_byfield',e))

    def read_document_bytwofield(self, fieldname1, fieldvalue1, fieldname2, fieldvalue2):
        try:
            _db = self.get_database()
            _colname = self.get_collection_name()
            _result = None
            if _db:
                if _colname:
                    _collection = _db[_colname]
                    if _collection:
                        _cursor = _collection.find({fieldname1: fieldvalue1, fieldname2:fieldvalue2})
                        for doc in _cursor:
                            _result = doc
            return _result     
        except Exception as e:
            print('====== %s.%s: %s' % ('db_util','read_document_byfields',e))

    def read_documents_bytwofield(self, fieldname1, fieldvalue1, fieldname2, fieldvalue2):
        try:
            _db = self.get_database()
            _colname = self.get_collection_name()
            _result = None
            if _db:
                if _colname:
                    ...
                    if _collection:
                        ...
                        #_cursor = _collection.find({'_id': ObjectId(id_string), fieldname:fieldvalue }) 
                        #_cursor = _collection.find( {"$and": [{fieldname1: fieldvalue1}, {fieldname2: fieldvalue2}]} ) 
                        #_cursor = _collection.find({'_id': ObjectId(id_string)},{ fieldname:fieldvalue })  
                        _cursor = _collection.find({fieldname1: fieldvalue1, fieldname2:fieldvalue2})
                        #for doc in _cursor:
                        #    _result = doc
...
def getFieldDatetime( doc, key):
    ...
    if doc != None:
        if type(doc) == dict:
            if doc.get(key) != None:
                if doc[key] != None:
                    #returndata = datetime.datetime.strptime(doc[key],'%Y-%m-%d')
                    returndata = doc[key]
    return returndata

def setFieldDatetime( doc, key):
    returndata = None
    if doc != None:
        if type(doc) == dict:
            if doc.get(key) != None:
                if doc[key] != None:
                    returndata = datetime.datetime.strptime(doc[key],'%Y-%m-%dT%H:%M:%SZ')
                    #returndata = doc[key]
    return returndata
